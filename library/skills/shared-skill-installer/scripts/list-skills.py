...
def _list_skills(repo: str, path: str, ref: str) -> list[str]:
    ...
    try:
        ...
        return _list_skills_from_zip(repo, path, ref, exc)
    try:
        data = json.loads(payload.decode("utf-8"))
    except json.JSONDecodeError as exc:
        return _list_skills_from_zip(repo, path, ref, exc)
    if not isinstance(data, list):
        return _list_skills_from_zip(repo, path, ref, None)
    skills = [item["name"] for item in data if item.get("type") == "dir"]
    return sorted(skills)


def _list_skills_from_zip(repo: str, path: str, ref: str, original_error: Exception | None) -> list[str]:
    zip_url = f"https://codeload.github.com/{repo}/zip/{ref}"
    try:
        payload = _request(zip_url)
    except urllib.error.HTTPError as exc:
        if isinstance(original_error, urllib.error.HTTPError) and original_error.code == 404:
            raise ListError(
